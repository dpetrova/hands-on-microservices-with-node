...
db.connect((err) => {
	...
	app.get("/uploads/:image", (req, res) => {
		//get the original image, untouched if no query params are provided
		...
		//process image accordingly provided query
	...
	app.get("/stats", (req, res) => {
		db.query("SELECT COUNT(*) total, SUM(size) size, MAX(date_used) last_used FROM images", (err, rows) => {
			if (err) {
	...
	//check our database periodically and delete older images
	setInterval(() => {
	...
	//to stop docker container gracefully
	process.on("SIGTERM", () => {
		db.end(() => {
			process.exit(0);
		});
